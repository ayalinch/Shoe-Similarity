...
Upload a photo of a shoe/boot sole, point the app at a folder of reference
sole photos (your "library"), and see the closest matches ranked by
...
from engine import IDENTICAL_THRESH, LIKELY_THRESH, extract_features
from library import ShoeLibrary
...
class DetailWindow(tk.Toplevel):
    """Full comparison card for one query-vs-match pair, with a Save button."""

    def __init__(self, master, query_features, match_result):
        super().__init__(master)
        self.match_result = match_result
        self.title(f"Comparison — {match_result.name}")
        self.geometry("960x760")

        card_bgr = build_pair_card(
            query_features, match_result.features,
            match_result.score, match_result.sub,
            title_a="Query photo", title_b=match_result.name,
        )
